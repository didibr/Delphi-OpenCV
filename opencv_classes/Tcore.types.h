...
public:
	...
	{
		FOutputArray.create(*sz->getSize(),type,i,allowTransposed, fixedDepthMask);
	}
    void __stdcall create(int rows, int cols, int type, int i, BOOL allowTransposed, int fixedDepthMask) 
	{
		FOutputArray.create(rows,cols,type,i,allowTransposed, fixedDepthMask);
	}
    void __stdcall create(int dims, int* size, int type, int i, BOOL allowTransposed, int fixedDepthMask)
	{
		FOutputArray.create(dims,size,type,i,allowTransposed, fixedDepthMask);
	}
